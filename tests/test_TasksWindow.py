...
class TestAddTaskWindow(unittest.TestCase):
    def setUp(self):
        """Создает родительское окно и пример данных."""
        self.parent = tk.Tk()
    ...
    def tearDown(self):
        """Закрывает родительское окно после теста."""
        self.parent.destroy()

    @patch("sqlite3.connect")
    def test_add_task_valid_data(self, mock_connect):
        """Тестирует добавление задачи с валидными данными."""
        mock_conn = MagicMock()
...
class TestEditTaskWindow(unittest.TestCase):
    ...
    def test_update_task_valid_data(self, mock_connect):
        """Тестирует обновление задачи с валидными данными."""
        mock_conn = MagicMock()
        mock_cursor = MagicMock()
        mock_conn.cursor.return_value = mock_cursor
        mock_connect.return_value = mock_conn
    ...
    def test_get_what_insert_in_date_notif(self):
        """Тестирует метод get_what_insert_in_date_notif."""
        window = EditTaskWindow(self.parent, self.task_example)
        self.assertEqual(
